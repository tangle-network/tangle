...
pub use rpc_core_trace::{FilterRequest, TraceServer};
use rpc_core_types::{RequestBlockId, RequestBlockTag};
use rpc_primitives_debug::DebugRuntimeApi;
	...
	C::Api: ApiExt<B>,
{
	...
	fn cache_block(
		...
		proxy.using(f)?;
		let mut traces: Vec<_> = client_evm_tracing::formatters::TraceFilter::format(proxy)
