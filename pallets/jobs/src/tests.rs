...
use tangle_primitives::jobs::{
	DKGPhaseOneJobType, DKGPhaseTwoJobType, DKGSignatureResult, JobSubmission, JobType,
};
...
fn jobs_submission_e2e_works_for_dkg() {
	new_test_ext().execute_with(|| {
		...
		let submission = JobSubmission {
			expiry: 100,
			job_type: JobType::DKGPhaseOne(DKGPhaseOneJobType {
		...
		let submission = JobSubmission {
			expiry: 100,
			job_type: JobType::DKGPhaseTwo(DKGPhaseTwoJobType {
				phase_one_id: 0,
				submission: vec![],
			}),
		};
