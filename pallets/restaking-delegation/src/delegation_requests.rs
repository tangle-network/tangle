...
impl<T: Config> Pallet<T> {
	/// Schedules a [DelegationAction::Revoke] for the delegator, towards a given operator.
	pub(crate) fn delegation_schedule_revoke(
		operator: T::AccountId,
		delegator: T::AccountId,
	) -> DispatchResultWithPostInfo {
		let mut state = <DelegatorState<T>>::get(&delegator).ok_or(<Error<T>>::DelegatorDNE)?;
		let mut scheduled_requests = <DelegationScheduledRequests<T>>::get(&operator);
		...
		let bonded_amount = state.get_bond_amount(&operator).ok_or(<Error<T>>::DelegationDNE)?;
		...
		<DelegationScheduledRequests<T>>::insert(operator.clone(), scheduled_requests);
	...
	/// Schedules a [DelegationAction::Decrease] for the delegator, towards a given operator.
	pub(crate) fn delegation_schedule_bond_decrease(
		...
		let bonded_amount = state.get_bond_amount(&operator).ok_or(DispatchErrorWithPostInfo {
			post_info: Some(actual_weight).into(),
	...
	/// Cancels the delegator's existing [ScheduledRequest] towards a given operator.
	...
	/// Executes the delegator's existing [ScheduledRequest] towards a given operator.
	pub(crate) fn delegation_execute_scheduled_request(
		...
		match request.action {
			DelegationAction::Revoke(amount) => {
				...
				state.rm_delegation::<T>(&operator);
				...
				<DelegationScheduledRequests<T>>::insert(operator, scheduled_requests);
				if leaving {
	...
	/// Removes the delegator's existing [ScheduledRequest] towards a given operator, if exists.
	...
	pub fn delegation_request_exists(operator: &T::AccountId, delegator: &T::AccountId) -> bool {
		<DelegationScheduledRequests<T>>::get(operator)
			.iter()
	...
	pub fn delegation_request_revoke_exists(
		...
		<DelegationScheduledRequests<T>>::get(operator).iter().any(|req| {
