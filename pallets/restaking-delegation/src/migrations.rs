	...
	BlockNumberFor<T>: From<u32> + Into<u64>,
{
	...
	fn pre_upgrade() -> Result<Vec<u8>, sp_runtime::TryRuntimeError> {
		...
		let len = maybe_raw_value.expect("RestakingDelegation.Round should exist!").len();
		ensure!(
			len == 12 || len == 16,
			"RestakingDelegation.Round should have 12 or 16 bytes length!"
		);
	...
	fn on_runtime_upgrade() -> frame_support::pallet_prelude::Weight {
			...
			unhashed::get_raw(&raw_key)
		{
			...
			match len {
				...
				// Storage corrupted
				x => panic!(
					"corrupted storage: RestakingDelegation.Round invalid length: {} bytes",
					x
				),
			}
		} else {
			panic!("corrupted storage: RestakingDelegation.Round don't exist");
		};
...
mod tests {
	...
	fn test_compute_theoretical_first_slot() {
		assert_eq!(compute_theoretical_first_slot::<u32>(10, 5, 100, 12_000), 90,);
	}
}
