...
use crate::{
	...
	OperatorStatus, Range, DELEGATOR_LOCK_ID,
};
...
fn invalid_root_origin_fails() {
	ExtBuilder::default().build().execute_with(|| {
		assert_noop!(
			RestakingDelegation::set_total_selected(RuntimeOrigin::signed(45), 6u32),
		...
		assert_noop!(
			RestakingDelegation::set_blocks_per_round(RuntimeOrigin::signed(45), 3u32),
			sp_runtime::DispatchError::BadOrigin
		);
	});
}
...
fn set_total_selected_event_emits_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(RestakingDelegation::set_blocks_per_round(RuntimeOrigin::root(), 7u32));
		roll_blocks(1);
		assert_ok!(RestakingDelegation::set_total_selected(RuntimeOrigin::root(), 6u32));
...
fn set_total_selected_fails_if_above_blocks_per_round() {
	ExtBuilder::default().build().execute_with(|| {
		assert_eq!(RestakingDelegation::round().length, 5); // test relies on this
		assert_noop!(
			RestakingDelegation::set_total_selected(RuntimeOrigin::root(), 6u32),
...
fn set_total_selected_fails_if_above_max_candidates() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			RestakingDelegation::set_total_selected(RuntimeOrigin::root(), 201u32),
...
fn set_total_selected_fails_if_equal_to_blocks_per_round() {
	ExtBuilder::default().build().execute_with(|| {
		assert_ok!(RestakingDelegation::set_blocks_per_round(RuntimeOrigin::root(), 10u32));
		assert_noop!(
			RestakingDelegation::set_total_selected(RuntimeOrigin::root(), 10u32),
			Error::<Test>::RoundLengthMustBeGreaterThanTotalSelectedCollators,
		);
	});
}
...
fn set_total_selected_passes_if_below_blocks_per_round() {
	ExtBuilder::default().build().execute_with(|| {
		assert_ok!(RestakingDelegation::set_blocks_per_round(RuntimeOrigin::root(), 10u32));
		assert_ok!(RestakingDelegation::set_total_selected(RuntimeOrigin::root(), 9u32));
	});
}
...
fn set_blocks_per_round_fails_if_below_total_selected() {
	ExtBuilder::default().build().execute_with(|| {
		assert_ok!(RestakingDelegation::set_blocks_per_round(RuntimeOrigin::root(), 20u32));
		assert_ok!(RestakingDelegation::set_total_selected(RuntimeOrigin::root(), 10u32));
		assert_noop!(
			RestakingDelegation::set_blocks_per_round(RuntimeOrigin::root(), 9u32),
			Error::<Test>::RoundLengthMustBeGreaterThanTotalSelectedCollators,
		);
	});
}
...
fn set_blocks_per_round_passes_if_above_total_selected() {
	ExtBuilder::default().build().execute_with(|| {
		assert_eq!(RestakingDelegation::round().length, 5); // test relies on this
		assert_ok!(RestakingDelegation::set_blocks_per_round(RuntimeOrigin::root(), 6u32));
	});
}
...
fn set_total_selected_storage_updates_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(RestakingDelegation::total_selected(), 5u32);
		assert_ok!(RestakingDelegation::set_total_selected(RuntimeOrigin::root(), 6u32));
		assert_eq!(RestakingDelegation::total_selected(), 6u32);
	});
}
...
fn cannot_set_total_selected_to_current_total_selected() {
	ExtBuilder::default().build().execute_with(|| {
		assert_noop!(
			RestakingDelegation::set_total_selected(RuntimeOrigin::root(), 5u32),
...
fn cannot_set_total_selected_below_module_min() {
	ExtBuilder::default().build().execute_with(|| {
		assert_noop!(
			RestakingDelegation::set_total_selected(RuntimeOrigin::root(), 4u32),
...
fn set_blocks_per_round_storage_updates_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		assert_eq!(RestakingDelegation::round().length, 5);
		assert_ok!(RestakingDelegation::set_blocks_per_round(RuntimeOrigin::root(), 6u32));
		assert_eq!(RestakingDelegation::round().length, 6);
	});
}
...
fn cannot_set_blocks_per_round_below_module_min() {
	ExtBuilder::default().build().execute_with(|| {
		assert_noop!(
			RestakingDelegation::set_blocks_per_round(RuntimeOrigin::root(), 2u32),
			Error::<Test>::CannotSetBelowMin
		);
	});
}
...
fn cannot_set_blocks_per_round_to_current_blocks_per_round() {
	ExtBuilder::default().build().execute_with(|| {
		assert_noop!(
			RestakingDelegation::set_blocks_per_round(RuntimeOrigin::root(), 5u32),
			Error::<Test>::NoWritingSameValue
		);
	});
}
...
fn invalid_monetary_origin_fails() {
	ExtBuilder::default().build().execute_with(|| {
		assert_noop!(
			RestakingDelegation::set_staking_expectations(
				RuntimeOrigin::signed(45),
		...
		assert_noop!(
			RestakingDelegation::set_inflation(
				RuntimeOrigin::signed(45),
				Range {
		...
		assert_noop!(
			RestakingDelegation::set_parachain_bond_account(RuntimeOrigin::signed(45), 11),
			sp_runtime::DispatchError::BadOrigin
		);
		assert_noop!(
			RestakingDelegation::set_parachain_bond_reserve_percent(
				RuntimeOrigin::signed(45),
...
fn set_staking_event_emits_event_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		// valid call succeeds
		assert_ok!(RestakingDelegation::set_staking_expectations(
			RuntimeOrigin::root(),
...
fn set_staking_updates_storage_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		assert_eq!(
			RestakingDelegation::inflation_config().expect,
...
fn set_inflation_event_emits_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(RestakingDelegation::set_inflation(
			RuntimeOrigin::root(),
			Range { min, ideal, max }
		));
...
fn set_inflation_storage_updates_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		let (min, ideal, max): (Perbill, Perbill, Perbill) =
			(Perbill::from_percent(3), Perbill::from_percent(4), Perbill::from_percent(5));
		assert_eq!(
			RestakingDelegation::inflation_config().annual,
			Range {
		...
		assert_eq!(
			RestakingDelegation::inflation_config().round,
			Range {
		...
		assert_eq!(RestakingDelegation::inflation_config().annual, Range { min, ideal, max });
		assert_eq!(
			RestakingDelegation::inflation_config().round,
			Range {
...
fn cannot_set_same_inflation() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			RestakingDelegation::set_inflation(RuntimeOrigin::root(), Range { min, ideal, max }),
			Error::<Test>::NoWritingSameValue
		);
	});
}
...
fn set_parachain_bond_account_event_emits_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		assert_ok!(RestakingDelegation::set_parachain_bond_account(RuntimeOrigin::root(), 11));
...
fn set_parachain_bond_account_storage_updates_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		assert_eq!(RestakingDelegation::parachain_bond_info().account, 0);
		assert_ok!(RestakingDelegation::set_parachain_bond_account(RuntimeOrigin::root(), 11));
		assert_eq!(RestakingDelegation::parachain_bond_info().account, 11);
	});
}
...
fn set_parachain_bond_reserve_percent_event_emits_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		assert_ok!(RestakingDelegation::set_parachain_bond_reserve_percent(
			RuntimeOrigin::root(),
...
fn set_parachain_bond_reserve_percent_storage_updates_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		assert_eq!(RestakingDelegation::parachain_bond_info().percent, Percent::from_percent(30));
		assert_ok!(RestakingDelegation::set_parachain_bond_reserve_percent(
			RuntimeOrigin::root(),
			Percent::from_percent(50)
		));
		assert_eq!(RestakingDelegation::parachain_bond_info().percent, Percent::from_percent(50));
	});
}
...
fn join_candidates_event_emits_correctly() {
	ExtBuilder::default().with_balances(vec![(1, 10)]).build().execute_with(|| {
		assert_ok!(RestakingDelegation::join_candidates(RuntimeOrigin::signed(1), 10u128, 0u32));
...
fn join_candidates_creates_candidate_state() {
	ExtBuilder::default().with_balances(vec![(1, 10)]).build().execute_with(|| {
		assert!(RestakingDelegation::candidate_info(1).is_none());
		assert_ok!(RestakingDelegation::join_candidates(RuntimeOrigin::signed(1), 10u128, 0u32));
		let candidate_state =
			RestakingDelegation::candidate_info(1).expect("just joined => exists");
...
fn join_candidates_adds_to_candidate_pool() {
	ExtBuilder::default().with_balances(vec![(1, 10)]).build().execute_with(|| {
		assert!(RestakingDelegation::candidate_pool().0.is_empty());
		assert_ok!(RestakingDelegation::join_candidates(RuntimeOrigin::signed(1), 10u128, 0u32));
		let candidate_pool = RestakingDelegation::candidate_pool();
...
fn cannot_join_candidates_if_candidate() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_noop!(
				RestakingDelegation::join_candidates(RuntimeOrigin::signed(1), 11u128, 100u32),
...
fn cannot_join_candidates_if_delegator() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_noop!(
				RestakingDelegation::join_candidates(RuntimeOrigin::signed(2), 10u128, 1u32),
...
fn cannot_join_candidates_without_min_bond() {
	ExtBuilder::default().with_balances(vec![(1, 1000)]).build().execute_with(|| {
		assert_noop!(
			RestakingDelegation::join_candidates(RuntimeOrigin::signed(1), 9u128, 100u32),
...
fn can_force_join_candidates_without_min_bond() {
	ExtBuilder::default().with_balances(vec![(1, 10)]).build().execute_with(|| {
		assert_ok!(RestakingDelegation::force_join_candidates(RuntimeOrigin::root(), 1, 9, 100u32));
		assert_events_eq!(Event::JoinedCollatorCandidates {
			account: 1,
			amount_locked: 9u128,
...
fn cannot_join_candidates_with_more_than_available_balance() {
	ExtBuilder::default().with_balances(vec![(1, 500)]).build().execute_with(|| {
		assert_noop!(
			RestakingDelegation::join_candidates(RuntimeOrigin::signed(1), 501u128, 100u32),
...
fn insufficient_join_candidates_weight_hint_fails() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			for i in 0..5 {
				assert_noop!(
					RestakingDelegation::join_candidates(RuntimeOrigin::signed(6), 20, i),
...
fn sufficient_join_candidates_weight_hint_succeeds() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			let mut count = 5u32;
			for i in 6..10 {
				assert_ok!(RestakingDelegation::join_candidates(
...
fn join_candidates_fails_if_above_max_candidate_count() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_noop!(
				RestakingDelegation::join_candidates(
...
fn leave_candidates_event_emits_correctly() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			assert_ok!(RestakingDelegation::schedule_leave_candidates(
...
fn leave_candidates_removes_candidate_from_candidate_pool() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			assert_eq!(RestakingDelegation::candidate_pool().0.len(), 1);
			assert_ok!(RestakingDelegation::schedule_leave_candidates(
				RuntimeOrigin::signed(1),
				1u32
			));
...
fn cannot_leave_candidates_if_not_candidate() {
	ExtBuilder::default().build().execute_with(|| {
		assert_noop!(
			RestakingDelegation::schedule_leave_candidates(RuntimeOrigin::signed(1), 1u32),
...
fn insufficient_leave_candidates_weight_hint_fails() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			for i in 1..6 {
				assert_noop!(
					RestakingDelegation::schedule_leave_candidates(RuntimeOrigin::signed(i), 4u32),
...
fn notify_inactive_collator_works() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(RestakingDelegation::notify_inactive_collator(RuntimeOrigin::signed(1), 1));
...
fn notify_inactive_collator_fails_too_low_collator_count() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			// Call 'notify_inactive_collator' extrinsic
			assert_noop!(
				RestakingDelegation::notify_inactive_collator(RuntimeOrigin::signed(1), 1),
...
fn notify_inactive_collator_fails_candidate_is_not_collator() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(RestakingDelegation::join_candidates(RuntimeOrigin::signed(6), 10, 100));
			...
			// to produce blocks
			assert_noop!(
				RestakingDelegation::notify_inactive_collator(RuntimeOrigin::signed(1), 6),
...
fn execute_leave_candidates_emits_event() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			roll_to(10);
			assert_ok!(RestakingDelegation::execute_leave_candidates(
				RuntimeOrigin::signed(1),
				1,
				0
			));
...
fn execute_leave_candidates_requires_correct_weight_hint() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			for i in 0..3 {
				assert_noop!(
					RestakingDelegation::execute_leave_candidates(RuntimeOrigin::signed(1), 1, i),
...
fn execute_leave_candidates_removes_candidate_state() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let candidate_state =
				RestakingDelegation::candidate_info(1).expect("just left => still exists");
			assert_eq!(candidate_state.bond, 10u128);
			roll_to(10);
			assert_ok!(RestakingDelegation::execute_leave_candidates(
				RuntimeOrigin::signed(1),
...
fn cannot_execute_leave_candidates_before_delay() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				RestakingDelegation::execute_leave_candidates(RuntimeOrigin::signed(3), 1, 0)
...
fn cancel_leave_candidates_emits_event() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(RestakingDelegation::cancel_leave_candidates(RuntimeOrigin::signed(1), 1));
...
fn cancel_leave_candidates_updates_candidate_state() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(RestakingDelegation::cancel_leave_candidates(RuntimeOrigin::signed(1), 1));
			let candidate =
				RestakingDelegation::candidate_info(&1).expect("just cancelled leave so exists");
...
fn cancel_leave_candidates_adds_to_candidate_pool() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(RestakingDelegation::cancel_leave_candidates(RuntimeOrigin::signed(1), 1));
			assert_eq!(RestakingDelegation::candidate_pool().0[0].owner, 1);
			assert_eq!(RestakingDelegation::candidate_pool().0[0].amount, 10);
		});
}
...
fn go_offline_updates_candidate_state_to_idle() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			let candidate_state =
				RestakingDelegation::candidate_info(1).expect("is active candidate");
			assert_eq!(candidate_state.status, OperatorStatus::Active);
			assert_ok!(RestakingDelegation::go_offline(RuntimeOrigin::signed(1)));
			let candidate_state =
				RestakingDelegation::candidate_info(1).expect("is candidate, just offline");
			assert_eq!(candidate_state.status, OperatorStatus::Idle);
		});
}
...
fn go_online_adds_to_candidate_pool() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(RestakingDelegation::go_online(RuntimeOrigin::signed(1)));
			assert_eq!(RestakingDelegation::candidate_pool().0[0].owner, 1);
			assert_eq!(RestakingDelegation::candidate_pool().0[0].amount, 20);
		});
}
...
fn go_online_storage_updates_candidate_state() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_ok!(RestakingDelegation::go_offline(RuntimeOrigin::signed(1)));
			let candidate_state =
				RestakingDelegation::candidate_info(1).expect("offline still exists");
			assert_eq!(candidate_state.status, OperatorStatus::Idle);
			assert_ok!(RestakingDelegation::go_online(RuntimeOrigin::signed(1)));
			let candidate_state = RestakingDelegation::candidate_info(1).expect("online so exists");
			assert_eq!(candidate_state.status, OperatorStatus::Active);
		});
}
...
fn cannot_go_online_if_leaving() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			assert_ok!(RestakingDelegation::schedule_leave_candidates(RuntimeOrigin::signed(1), 1));
			assert_noop!(
				RestakingDelegation::go_online(RuntimeOrigin::signed(1)).map_err(|err| err.error),
...
fn candidate_bond_more_emits_correct_event() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			assert_ok!(RestakingDelegation::candidate_bond_more(RuntimeOrigin::signed(1), 30));
...
fn candidate_bond_more_updates_candidate_state() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			let candidate_state =
				RestakingDelegation::candidate_info(1).expect("updated => exists");
...
fn candidate_bond_more_updates_candidate_pool() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(RestakingDelegation::candidate_bond_more(RuntimeOrigin::signed(1), 30));
			assert_eq!(RestakingDelegation::candidate_pool().0[0].owner, 1);
			assert_eq!(RestakingDelegation::candidate_pool().0[0].amount, 50);
		});
}
...
fn schedule_candidate_bond_less_event_emits_correctly() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			assert_ok!(RestakingDelegation::schedule_candidate_bond_less(
				RuntimeOrigin::signed(1),
				10
			));
...
fn cannot_schedule_candidate_bond_less_if_request_exists() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				RestakingDelegation::schedule_candidate_bond_less(RuntimeOrigin::signed(1), 5),
...
fn cannot_schedule_candidate_bond_less_if_not_candidate() {
	ExtBuilder::default().build().execute_with(|| {
		assert_noop!(
			RestakingDelegation::schedule_candidate_bond_less(RuntimeOrigin::signed(6), 50),
			Error::<Test>::CandidateDNE
		);
	});
}
...
fn cannot_schedule_candidate_bond_less_if_new_total_below_min_candidate_stk() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_noop!(
				RestakingDelegation::schedule_candidate_bond_less(RuntimeOrigin::signed(1), 21),
				Error::<Test>::CandidateBondBelowMin
			);
		});
}
...
fn cannot_schedule_candidate_bond_less_if_exited_candidates() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				RestakingDelegation::schedule_candidate_bond_less(RuntimeOrigin::signed(1), 10),
				Error::<Test>::CandidateDNE
			);
		});
}
...
fn execute_candidate_bond_less_emits_correct_event() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			roll_to(10);
			roll_blocks(1);
			assert_ok!(RestakingDelegation::execute_candidate_bond_less(
				RuntimeOrigin::signed(1),
				1
			));
...
fn execute_candidate_bond_less_updates_candidate_pool() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_eq!(RestakingDelegation::candidate_pool().0[0].owner, 1);
			assert_eq!(RestakingDelegation::candidate_pool().0[0].amount, 30);
			assert_ok!(RestakingDelegation::schedule_candidate_bond_less(
				RuntimeOrigin::signed(1),
				10
			));
...
fn cancel_candidate_bond_less_emits_event() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(RestakingDelegation::cancel_candidate_bond_less(RuntimeOrigin::signed(1)));
...
fn cancel_candidate_bond_less_updates_candidate_state() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(RestakingDelegation::cancel_candidate_bond_less(RuntimeOrigin::signed(1)));
			assert!(RestakingDelegation::candidate_info(&1).unwrap().request.is_none());
		});
}
...
fn only_candidate_can_cancel_candidate_bond_less_request() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				RestakingDelegation::cancel_candidate_bond_less(RuntimeOrigin::signed(2)),
				Error::<Test>::CandidateDNE
			);
		});
}
...
fn delegate_updates_collator_state() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			let candidate_state =
				RestakingDelegation::candidate_info(1).expect("registered in genesis");
			...
			assert_ok!(RestakingDelegation::delegate(RuntimeOrigin::signed(2), 1, 10, 0, 0));
			let candidate_state =
				RestakingDelegation::candidate_info(1).expect("just delegated => exists");
...
fn can_delegate_immediately_after_other_join_candidates() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			assert_ok!(RestakingDelegation::join_candidates(RuntimeOrigin::signed(1), 20, 0));
...
fn execute_revoke_delegation_removes_delegation_from_candidate_state() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_eq!(
				RestakingDelegation::candidate_info(1).expect("exists").delegation_count,
				1u32
			);
			assert_ok!(RestakingDelegation::schedule_revoke_delegation(
				RuntimeOrigin::signed(2),
			...
			assert!(RestakingDelegation::candidate_info(1)
				.expect("exists")
				.delegation_count
				.is_zero());
		});
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			let pre_call_candidate_info =
				RestakingDelegation::candidate_info(&1).expect("delegated by all so exists");
...
fn collator_exit_executes_after_delay() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			roll_to(11);
			assert_ok!(RestakingDelegation::schedule_leave_candidates(RuntimeOrigin::signed(2), 2));
			assert_events_eq!(Event::CandidateScheduledExit {
				exit_allowed_round: 3,
				candidate: 2,
				scheduled_exit: 5,
			});
			let info = RestakingDelegation::candidate_info(&2).unwrap();
...
fn collator_selection_chooses_top_candidates() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			roll_to_round_begin(2);
			assert_ok!(RestakingDelegation::schedule_leave_candidates(RuntimeOrigin::signed(6), 6));
...
fn multiple_delegations() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(RestakingDelegation::schedule_leave_candidates(RuntimeOrigin::signed(2), 5));
			assert_events_eq!(
				Event::Delegation {
					delegator: 7,
					locked_amount: 80,
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			fn is_candidate_pool_bond(account: u64, bond: u128) {
				let pool = RestakingDelegation::candidate_pool();
...
fn only_top_collators_are_counted() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let collator_state = RestakingDelegation::candidate_info(1).unwrap();
			// 15 + 16 + 17 + 18 + 20 = 86 (top 4 + self bond)
...
fn delegation_events_convey_correct_position() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			let collator1_state = RestakingDelegation::candidate_info(1).unwrap();
...
fn no_selected_candidates_defaults_to_last_round_collators() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let old_round = RestakingDelegation::round().current;
			let old_selected_candidates = RestakingDelegation::selected_candidates();
			...
			// next round
			roll_to_round_begin(4);
			let new_round = RestakingDelegation::round().current;
			// check AtStake matches previous
			let new_selected_candidates = RestakingDelegation::selected_candidates();
...
fn test_delegator_scheduled_for_revoke_is_rewarded_for_previous_rounds_but_not_for_future() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let collator = RestakingDelegation::candidate_info(1).expect("candidate must exist");
			assert_eq!(
...
fn test_hotfix_remove_delegation_requests_exited_candidates_cleans_up() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(RestakingDelegation::hotfix_remove_delegation_requests_exited_candidates(
				RuntimeOrigin::signed(1),
...
fn test_hotfix_remove_delegation_requests_exited_candidates_errors_when_requests_not_empty() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				RestakingDelegation::hotfix_remove_delegation_requests_exited_candidates(
					RuntimeOrigin::signed(1),
					vec![2, 3]
				),
...
fn test_execute_leave_candidates_removes_auto_compounding_state() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(RestakingDelegation::schedule_leave_candidates(RuntimeOrigin::signed(1), 2));
			roll_to(10);
			assert_ok!(RestakingDelegation::execute_leave_candidates(
				RuntimeOrigin::signed(1),
				1,
...
fn test_compute_top_candidates_is_stable() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(RestakingDelegation::candidate_pool().0.len(), 6);
			assert_eq!(RestakingDelegation::total_selected(), 5);
			// Returns the 5 candidates with greater AccountId, because they are iterated in reverse
			assert_eq!(RestakingDelegation::compute_top_candidates(), vec![2, 3, 4, 5, 6]);
		});
}
...
fn test_removed_calls() {
	ExtBuilder::default().build().execute_with(|| {
		assert_err!(
			RestakingDelegation::removed_call_19(RuntimeOrigin::root()),
			Error::<Test>::RemovedCall
		);
		assert_err!(
			RestakingDelegation::removed_call_20(RuntimeOrigin::root()),
			Error::<Test>::RemovedCall
		);
		assert_err!(
			RestakingDelegation::removed_call_21(RuntimeOrigin::root()),
			Error::<Test>::RemovedCall
		);
	});
}
