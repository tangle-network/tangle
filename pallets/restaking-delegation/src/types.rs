...
use crate::{
	...
	Total, DELEGATOR_LOCK_ID, OPERATOR_LOCK_ID,
};
use frame_support::{
	pallet_prelude::*,
...
impl<A: Clone, B: Copy> From<CollatorCandidate<A, B>> for OperatorSnapshot<A, B> {
	fn from(other: CollatorCandidate<A, B>) -> OperatorSnapshot<A, B> {
		OperatorSnapshot {
			bond: other.bond,
			delegations: other
				.top_delegations
