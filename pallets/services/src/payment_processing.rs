use crate::{
	Config, Error, Pallet, Instances, StagingServicePayments,
...
impl<T: Config> Pallet<T> {
	...
	pub fn process_service_payment(
		...
		match &blueprint.pricing_model {
			...
			PricingModel::EventDriven { reward_per_event } => {
				...
				log::debug!("Event-driven service {}, reward per event: {:?}", service_id, reward_per_event);
			},
		}
	...
	fn process_subscription_payment(
		...
		if let Some(end_block) = maybe_end {
			if current_block > end_block {
				log::debug!("Subscription for service {} has ended at block {:?}", service_id, end_block);
				return Ok(());
			}
		}
	...
	pub fn process_event_driven_payment(
		service_id: u64,
		event_count: u32,
	) -> DispatchResult {
		let service = Self::services(service_id)?;
		...
		match &staging_payment.asset {
			Asset::Custom(asset_id) => {
				if *asset_id == T::AssetId::default() {
					...
					let account_id = staging_payment.refund_to.clone().try_into_account_id()
		...
		for (service_id, _) in Instances::<T>::iter() {
			if let Ok(service) = Self::services(service_id) {
				if let Ok((_, blueprint)) = Self::blueprints(service.blueprint) {
					if let PricingModel::Subscription { rate_per_interval, interval, maybe_end } = &blueprint.pricing_model {
	...
	pub fn validate_payment_amount(
		...
		match &blueprint.pricing_model {
			...
			PricingModel::EventDriven { reward_per_event } => {
				...
				log::debug!("Event-driven service, reward per event: {:?}, provided: {:?}", reward_per_event, provided_amount);
			},
		}
