...
use k256::{
	...
	CompressedPoint, FieldBytes, ProjectivePoint, Scalar,
};
...
impl Decode for WrappedScalar {
	fn decode<I: parity_scale_codec::Input>(
		...
		let buffer = FieldBytes::from_slice(&bytes);
		Ok(WrappedScalar(Scalar::from_repr(*buffer).unwrap_or(Scalar::ZERO)))
	}
}
...
/// A wrapper around a [`k256::ProjectivePoint`] to implement the
...
impl Decode for WrappedProjectivePoint {
	fn decode<I: parity_scale_codec::Input>(
		...
		input.read(&mut bytes)?;
		let pt = CompressedPoint::from_slice(&bytes);
		Ok(WrappedProjectivePoint(
			ProjectivePoint::from_bytes(pt).unwrap_or(ProjectivePoint::default()),
		))
	}
}
