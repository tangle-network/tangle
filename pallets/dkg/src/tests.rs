...
use tangle_primitives::jobs::{DKGResult, DKGSignatureResult, JobResult};
...
fn dkg_key_verifcation_works() {
	new_test_ext().execute_with(|| {
		...
		let participant_one = mock_pub_key();
		let participant_two = mock_pub_key();
		...
		let job_to_verify = DKGResult {
			...
			participants: vec![participant_one, participant_two],
