use super::mock::*;
use crate::{
    types::{LockMultiplier, RewardConfig, RewardConfigForAssetVault, StakePoints},
    Error,
};
...
fn test_reward_distribution_and_claiming() {
    new_test_ext().execute_with(|| {
        // Setup stake points for ALICE
        assert_ok!(MultiAssetDelegation::set_lock_period(
            RuntimeOrigin::signed(ALICE),
            TNT_ASSET_ID,
            3
        ));
        MultiAssetDelegation::stake_points_mut(&ALICE, TNT_ASSET_ID, |points| {
            *points = Some(StakePoints {
                base_points: 1000,
                lock_multiplier: 2,
                expiry: 1000,
                auto_compound: false,
        ...
        assert_ok!(MultiAssetDelegation::claim_rewards(
            RuntimeOrigin::signed(ALICE),
            TNT_ASSET_ID
        ));
...
fn test_auto_compound_rewards() {
    new_test_ext().execute_with(|| {
        // Setup stake points with auto-compound enabled
        assert_ok!(MultiAssetDelegation::toggle_auto_compound(
            RuntimeOrigin::signed(ALICE),
            TNT_ASSET_ID
        ));
        ...
        let points = MultiAssetDelegation::stake_points(&ALICE, TNT_ASSET_ID).unwrap();
        assert!(points.base_points > 1000);
        assert_eq!(MultiAssetDelegation::pending_rewards(&ALICE, TNT_ASSET_ID), 0);
    });
}
...
fn test_claim_all_rewards() {
    new_test_ext().execute_with(|| {
        // Setup stake points for multiple assets
        MultiAssetDelegation::stake_points_mut(&ALICE, TNT_ASSET_ID, |points| {
            *points = Some(StakePoints {
                base_points: 1000,
                lock_multiplier: 1,
        ...
        MultiAssetDelegation::stake_points_mut(&ALICE, DOT_ASSET_ID, |points| {
            *points = Some(StakePoints {
                base_points: 1000,
                lock_multiplier: 1,
                expiry: 1000,
