                ...
                if let Some(vault_id) = AssetLookupRewardVaults::<T>::get(asset_id) {
                    if let Some(config) = reward_config.configs.get(&vault_id) {
                        ...
                        for delegation in delegations {
                            if let Some(stake_points) = StakePoints::<T>::get(&delegation.delegator, asset_id) {
                                ...
                                if asset_id == &T::NativeAssetId::get() {
                                    points = points.saturating_mul(config.tnt_boost_multiplier.into());
                                }
                        ...
                        if !total_points.is_zero() {
                            ...
                            let total_reward = Self::calculate_total_reward(config.apy, total_points)?;
                            ...
                            for (delegator, points) in delegation_points {
                                ...
                                if let Some(stake_points) = StakePoints::<T>::get(&delegator, asset_id) {
