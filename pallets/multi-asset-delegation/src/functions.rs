...
impl<T: Config> Pallet<T> {
    ...
    pub fn process_operator_bond_more(who: &T::AccountId, additional_bond: BalanceOf<T>) -> Result<(), DispatchError> {
    ...
    pub fn process_schedule_operator_bond_less(who: &T::AccountId, bond_less_amount: BalanceOf<T>) -> Result<(), DispatchError> {
    ...
    pub fn process_execute_operator_bond_less(who: &T::AccountId) -> Result<(), DispatchError> {
        ...
        let current_round = Self::current_round();
        ensure!(current_round >= T::LeaveOperatorsDelay::get() + request.request_time, Error::<T>::BondLessRequestNotSatisfied);
