...
use frame_support::{
	...
	traits::{
		ConstU128, ConstU32, Contains, EnsureOrigin, EnsureOriginWithArg, Everything, Nothing,
	},
	PalletId,
};
...
impl FixedConversionRateProvider for MyFixedConversionRateProvider {
	fn get_fee_per_second(location: &Location) -> Option<u128> {
		let metadata =
			AssetRegistry::fetch_metadata_by_location(&location.clone().try_into().unwrap())?;
