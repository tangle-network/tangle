	...
	fn convert(l: Location) -> Option<CurrencyId> {
		...
		let currency_id = match l.clone().unpack() {
			(parents, interior) if parents == 1 => match interior {
				...
				[Parachain(1), GeneralKey { data, .. }] if data.to_vec() == a1 => {
					Some(CurrencyId::A1)
				},
				[Parachain(2), GeneralKey { data, .. }] if data.to_vec() == b => {
					Some(CurrencyId::B)
				},
				[Parachain(2), GeneralKey { data, .. }] if data.to_vec() == b1 => {
					Some(CurrencyId::B1)
				},
				[Parachain(2), GeneralKey { data, .. }] if data.to_vec() == b2 => {
					Some(CurrencyId::B2)
				},
				[Parachain(4), GeneralKey { data, .. }] if data.to_vec() == d => {
					Some(CurrencyId::D)
				},
				_ => None,
			},
