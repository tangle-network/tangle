...
use orml_traits::{
	asset_registry::{
		AssetMetadata, AssetProcessor, FixedConversionRateProvider, Inspect, Mutate,
...
impl<W: WeightToFeeConverter, R: TakeRevenue> WeightTrader for AssetRegistryTrader<W, R> {
	fn new() -> Self {
		Self { bought_weight: None, _phantom: Default::default() }
	}
	...
	fn buy_weight(
		...
		for (asset, _) in payment.fungible.iter() {
			...
			if let Some(fee_increase) = W::convert_weight_to_fee(location, weight) {
				...
				if let Ok(unused) =
					payment.clone().checked_sub((asset.clone(), fee_increase).into())
				{
