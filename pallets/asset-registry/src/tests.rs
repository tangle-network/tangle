...
use crate::{
	tests::para::{
		AdminAssetTwo, AssetRegistry, CustomMetadata, RuntimeOrigin, Tokens, TreasuryAccount,
	},
...
> {
	let loc: VersionedLocation =
		Location::new(1, [Parachain(1), Junction::from(BoundedVec::try_from(vec![0]).unwrap())])
			.into();
	AssetMetadata {
		decimals: 12,
		...
		location: Some(loc),
		additional: CustomMetadata { fee_per_second: 1_000_000_000_000 },
	}
}
	...
	ParaA::execute_with(|| {
		...
		assert_ok!(ParaXTokens::transfer(
			...
			Box::new(
				Location::new(
					1,
					[Parachain(2), Junction::AccountId32 { network: None, id: BOB.into() }]
				)
				.into()
			),
...
fn send_sibling_asset_to_non_reserve_sibling() {
	...
	ParaB::execute_with(|| {
		AssetRegistry::register_asset(
			...
			AssetMetadata {
				location: Some(
					Location::new(0, [Junction::from(BoundedVec::try_from(vec![0]).unwrap())])
						.into(),
				),
				..dummy_metadata()
			},
	...
	ParaA::execute_with(|| {
		assert_ok!(ParaXTokens::transfer(
			...
			Box::new(
				Location::new(
					1,
					[Parachain(3), Junction::AccountId32 { network: None, id: BOB.into() }]
				)
				.into()
			),
			WeightLimit::Unlimited
...
fn test_sequential_id_with_invalid_id_returns_error() {
	...
	ParaA::execute_with(|| {
		assert_ok!(AssetRegistry::register_asset(RuntimeOrigin::root(), dummy_metadata(), Some(1)));
		assert_noop!(
...
fn test_fixed_rate_asset_trader() {
	...
	ParaB::execute_with(|| {
		...
		AssetRegistry::update_asset(
			...
			None,
			None,
			Some(CustomMetadata { fee_per_second: metadata.additional.fee_per_second * 2 }),
		)
		.unwrap();
	});
...
fn test_register_duplicate_location_returns_error() {
	...
	ParaA::execute_with(|| {
		...
		let register_asset =
			RuntimeCall::AssetRegistry(crate::Call::<para::Runtime>::register_asset {
				metadata,
				asset_id: None,
			});
		assert_noop!(
...
fn test_update_metadata_works() {
	...
	ParaA::execute_with(|| {
		...
		let new_metadata = AssetMetadata {
			...
			additional: CustomMetadata { fee_per_second: 2_000_000_000_000 },
		};
...
fn test_existential_deposits() {
	...
	ParaA::execute_with(|| {
		let metadata = AssetMetadata { existential_deposit: 100, ..dummy_metadata() };
		...
		assert_ok!(Tokens::transfer(Some(ALICE).into(), BOB, CurrencyId::RegisteredAsset(1), 100));
...
fn test_v2_to_v3_incompatible_multilocation() {
	...
	#[allow(deprecated)]
	let old_location =
		OldLocation::new(0, OldJunctions::X1(OldJunction::GeneralKey(vec![0].try_into().unwrap())));
	assert!(
		old_location.encode()
			!= Location::new(0, [Junction::from(BoundedVec::try_from(vec![0]).unwrap())]).encode()
	);
}
