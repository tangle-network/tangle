...
mod tests {
	...
	async fn test_job_submission_event() {
		...
		let alice_role_key_seed =
			"0x4141414141414141414141414141414141414141414141414141414141414141";
		let bob_role_key_seed =
			"0x4242424242424242424242424242424242424242424242424242424242424242";

		let alice_role_key = sp_core::ecdsa::Pair::from_string(&alice_role_key_seed, None).unwrap();
		println!("Alice role key: {:?}", alice_role_key.public());

		let bob_role_key = sp_core::ecdsa::Pair::from_string(&bob_role_key_seed, None).unwrap();
		...
		let session_key_query =
			tangle_runtime::api::storage().session().next_keys(alice_account_id.clone());
		let session_key = subxt_client
			.storage()
			.at_latest()
			.await
			.unwrap()
		...
		println!("Role Session key for alice : {:?}", hex::encode(session_key.role.0 .0.to_vec()));
		println!("Role Session key for alice : {:?}", session_key.role);
		assert_eq!(1, 2);
		...
		let jobs_tx = tangle_runtime::api::tx()
			.jobs()
		...
		let job_result_tx = tangle_runtime::api::tx()
			.jobs()
			.submit_job_result(
