...
pallet_staking_reward_curve::build! {
	const REWARD_CURVE: PiecewiseLinear<'static> = curve!(
		...
		ideal_stake: 0_800_000, // ideal stake (80% of total supply)
		falloff: 0_050_000,
		max_piece_count: 40,
...
impl pallet_services::Config for Runtime {
	...
	type Currency = Balances;
	type Fungibles = Assets;
	type PalletEvmAccount = TangleServicesAccountId;
	type EvmRunner = EVM;
	type EvmGasWeightMapping = TangleGasWeightMapping;
	type EvmAddressMapping = TangleEvmAddressMapping<Runtime>;
	type AssetId = AssetId;
	type RoleKeyId = TangleRoleKeyId;
	type MaxFields = TangleMaxFields;
	type MaxFieldsSize = TangleMaxFieldsSize;
	type MaxMetadataLength = TangleMaxMetadataLength;
	type WeightInfo = pallet_services::weights::SubstrateWeight<Runtime>;
	type MaxPermittedCallers = TangleMaxPermittedCallers;
	type MaxOperators = TangleMaxOperators;
	type MaxServicesPerUser = TangleMaxServicesPerUser;
	type MaxBlueprintsPerUser = TangleMaxBlueprintsPerUser;
	type OperatorDelegationManager = OperatorDelegation;
	type MasterBlueprintServiceManagerUpdateOrigin = EnsureRootOrHalfCouncil;
	type MaxMasterBlueprintServiceManagerVersions = TangleMaxMasterBlueprintServiceManagerVersions;
	type SlashManager = Treasury;
	type RewardsManager = Rewards; // Assuming this remains for other potential reward logic
	type RewardRecorder = Rewards; // Add the link to the Rewards pallet
}
