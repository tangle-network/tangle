...
impl<BlockNumber, Balance> BillingOperations<BlockNumber, Balance>
	...
	Balance: Copy + Zero + Saturating + CheckedMul + CheckedAdd + PartialOrd + From<u64>,
{
	fn calculate_billing(
		...
		use super::types::{BillingSkipReason, BillingTrigger};

		match self {
			PricingModel::PayOnce { .. } => self
				.calculate_pay_once_billing(billing_state.last_billed)
			...
			PricingModel::Subscription { .. } => self
				.calculate_subscription_billing(current_block, billing_state.last_billed)
				.unwrap_or_else(|| BillingCalculation {
					amount: Balance::zero(),
			...
			PricingModel::EventDriven { .. } => self
				.calculate_event_driven_billing(&billing_state.event_log)
				.unwrap_or_else(|| BillingCalculation {
					amount: Balance::zero(),
