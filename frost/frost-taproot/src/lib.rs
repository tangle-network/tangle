...
impl Ciphersuite for Secp256K1Taproot {
	...
	fn H2(m: &[u8]) -> <<Self::Group as Group>::Field as Field>::Scalar {
		...
		WrappedScalar(<Scalar as Reduce<k256::U256>>::reduce_bytes(
			&tagged_hash(CHALLENGE_TAG).chain_update(m).finalize(),
		))
	}

	/// H3 for FROST(secp256k1, Taproot)
	///
...
mod tests {
	...
	fn test_sign_and_verify() {
		...
		let sk = SigningKey::<Secp256K1Taproot>::new(&mut rng);
		let vk = VerifyingKey::<Secp256K1Taproot>::from(sk);
