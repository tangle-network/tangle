		...
		.execute_with(|| {
			PrecompilesValue::get()
				.prepare_test(
					Alice,
					Precompile1,
					PCall::add_proxy { delegate: Address(Bob.into()), proxy_type: 10, delay: 0 },
				)
		...
		.execute_with(|| {
			...
			PrecompilesValue::get()
				.prepare_test(
					Alice,
					Precompile1,
					PCall::remove_proxy { delegate: Address(Bob.into()), proxy_type: 10, delay: 0 },
				)
				.execute_reverts(|o| o == b"proxyType: Failed decoding value to ProxyType");
		})
}
		...
		.execute_with(|| {
			...
			PrecompilesValue::get()
				.prepare_test(
					Alice,
					Precompile1,
					PCall::add_proxy { delegate: Address(Bob.into()), proxy_type: 2, delay: 0 },
				)
				.execute_returns(());
		...
		.execute_with(|| {
			...
			PrecompilesValue::get()
				...
				.with_subcall_handle(move |subcall| {
					let Subcall { address, transfer, input, target_gas: _, is_static, context } =
						subcall;
