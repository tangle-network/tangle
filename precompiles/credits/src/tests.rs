...
fn burn_works() {
	ExtBuilder::default()
		.balances(vec![(accounts().get("alice").unwrap().into(), 10_000_000)])
		.build()
		.execute_with(|| {
			...
			precompiles()
				...
				.expect_success()
				.expect_log(
					pallet_credits::Event::CreditsGrantedFromBurn {
						who: alice.into(),
			...
			assert_eq!(Balances::free_balance(alice.into()), 10_000_000 - amount as u128);
		});
}
...
fn burn_insufficient_balance_fails() {
	ExtBuilder::default()
		.balances(vec![(accounts().get("alice").unwrap().into(), 1000)])
		.build()
		.execute_with(|| {
			let alice = accounts().get("alice").unwrap();
			...
			precompiles()
				...
				.execute_reverts(|output| {
					from_utf8_lossy(&output).contains("InsufficientTntBalance")
				});
		});
}
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			precompiles()
				...
				.expect_success()
				.expect_log(
					pallet_credits::Event::CreditsClaimed {
						who: alice.into(),
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			precompiles()
				...
				.execute_reverts(|output| {
					from_utf8_lossy(&output).contains("ClaimAmountExceedsWindowAllowance")
				});
		});
}
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		let result = precompiles()
			.prepare_test(
				alice,
				PRECOMPILE_ADDRESS,
				EvmDataWriter::new_with_selector(Action::GetStakeTiers).build(),
			)
			.expect_success()
			.execute_returns_raw();
