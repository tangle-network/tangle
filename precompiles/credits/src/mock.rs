...
impl MockPrecompile {
	pub fn new_mock_precompile(context: &Context) -> Box<dyn PrecompileHandle> {
...
pub const PRECOMPILE_ADDRESS: Address =
	Address(H160::from_str("0x0000000000000000000000000000000000000800").unwrap());

pub type Precompiles<R> =
	PrecompileSetBuilder<R, (PrecompileAt<PRECOMPILE_ADDRESS, CreditsPrecompile<R>>,)>;
...
impl Default for ExtBuilder {
	fn default() -> ExtBuilder {
		ExtBuilder {
			balances: vec![],
			stake_tiers: vec![(100_000, 10), (500_000, 50), (1_000_000, 100)],
		}
	}
}
...
impl ExtBuilder {
	...
	pub(crate) fn build(self) -> sp_io::TestExternalities {
		...
		for (threshold, rate_per_block) in self.stake_tiers {
			stake_tiers.push(StakeTier { threshold, rate_per_block });
		}

		pallet_credits::GenesisConfig::<Runtime> { stake_tiers }
			.assimilate_storage(&mut t)
			.unwrap();
