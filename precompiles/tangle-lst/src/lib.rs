...
use pallet_tangle_lst::{PoolId, PoolState, BondExtra};
    ...
    Runtime::AccountId: From<WrappedAccountId32>,
{
    ...
    fn bond_extra(handle: &mut impl PrecompileHandle, pool_id: U256, extra_type: u8, extra: U256) -> EvmResult {
        handle.record_cost(RuntimeHelper::<Runtime>::db_read_gas_cost())?;
        let origin = Runtime::AddressMapping::into_account_id(handle.context().caller);
        
        let pool_id: PoolId = pool_id.try_into().map_err(|_| revert("Invalid pool id"))?;
        let extra: BalanceOf<Runtime> = extra.try_into().map_err(|_| revert("Invalid extra amount"))?;
    ...
    fn unbond(handle: &mut impl PrecompileHandle, member_account: H256, pool_id: U256, unbonding_points: U256) -> EvmResult {
        handle.record_cost(RuntimeHelper::<Runtime>::db_read_gas_cost())?;
        let origin = Runtime::AddressMapping::into_account_id(handle.context().caller);
        
        let member_account = Self::convert_to_account_id(member_account)?;
        let pool_id: PoolId = pool_id.try_into().map_err(|_| revert("Invalid pool id"))?;
        let unbonding_points: BalanceOf<Runtime> = unbonding_points.try_into().map_err(|_| revert("Invalid unbonding points"))?;

        let call = pallet_tangle_lst::Call::<Runtime>::unbond { member_account, pool_id, unbonding_points };
    ...
    fn pool_withdraw_unbonded(handle: &mut impl PrecompileHandle, pool_id: U256, num_slashing_spans: u32) -> EvmResult {
        handle.record_cost(RuntimeHelper::<Runtime>::db_read_gas_cost())?;
        let origin = Runtime::AddressMapping::into_account_id(handle.context().caller);
        
        let pool_id: PoolId = pool_id.try_into().map_err(|_| revert("Invalid pool id"))?;

        let call = pallet_tangle_lst::Call::<Runtime>::pool_withdraw_unbonded { pool_id, num_slashing_spans };
    ...
    fn withdraw_unbonded(handle: &mut impl PrecompileHandle, member_account: H256, pool_id: U256, num_slashing_spans: u32) -> EvmResult {
        handle.record_cost(RuntimeHelper::<Runtime>::db_read_gas_cost())?;
        let origin = Runtime::AddressMapping::into_account_id(handle.context().caller);
        ...
        let call = pallet_tangle_lst::Call::<Runtime>::withdraw_unbonded { member_account, pool_id, num_slashing_spans };
    ...
    fn create(handle: &mut impl PrecompileHandle, amount: U256, root: H256, nominator: H256, bouncer: H256) -> EvmResult {
        handle.record_cost(RuntimeHelper::<Runtime>::db_read_gas_cost())?;
        let origin = Runtime::AddressMapping::into_account_id(handle.context().caller);
    ...
    fn create_with_pool_id(handle: &mut impl PrecompileHandle, amount: U256, root: H256, nominator: H256, bouncer: H256, pool_id: U256) -> EvmResult {
        handle.record_cost(RuntimeHelper::<Runtime>::db_read_gas_cost())?;
        let origin = Runtime::AddressMapping::into_account_id(handle.context().caller);
        ...
        let call = pallet_tangle_lst::Call::<Runtime>::create_with_pool_id { amount, root, nominator, bouncer, pool_id };
    ...
    fn nominate(handle: &mut impl PrecompileHandle, pool_id: U256, validators: Vec<H256>) -> EvmResult {
        handle.record_cost(RuntimeHelper::<Runtime>::db_read_gas_cost())?;
        let origin = Runtime::AddressMapping::into_account_id(handle.context().caller);
        
        let pool_id: PoolId = pool_id.try_into().map_err(|_| revert("Invalid pool id"))?;
        let validators: Vec<Runtime::AccountId> = validators.into_iter().map(Self::convert_to_account_id).collect::<Result<_, _>>()?;
    ...
    fn set_metadata(handle: &mut impl PrecompileHandle, pool_id: U256, metadata: Vec<u8>) -> EvmResult {
        handle.record_cost(RuntimeHelper::<Runtime>::db_read_gas_cost())?;
        let origin = Runtime::AddressMapping::into_account_id(handle.context().caller);
    ...
    fn set_configs(
        ...
        let call = pallet_tangle_lst::Call::<Runtime>::set_configs {
            ...
            global_max_commission: global_max_commission.map(ConfigOp::Set).unwrap_or(ConfigOp::Noop),
        };
