...
pub mod tracer {
	...
	impl EvmTracer {
		pub fn new() -> Self {
			Self { step_event_filter: primitives_ext::ext::step_event_filter() }
		}
		...
		pub fn emit_new() {
			primitives_ext::ext::call_list_new();
		}
	}
	...
	impl EvmListener for EvmTracer {
		...
		fn event(&mut self, event: evm::tracing::Event) {
			...
			primitives_ext::ext::evm_event(message);
		}
	}
	...
	impl GasometerListener for EvmTracer {
		...
		fn event(&mut self, event: evm_gasometer::tracing::Event) {
			...
			let message = event.encode();
			primitives_ext::ext::gasometer_event(message);
		}
	}
	...
	impl RuntimeListener for EvmTracer {
		...
		fn event(&mut self, event: evm_runtime::tracing::Event) {
			...
			let message = event.encode();
			primitives_ext::ext::runtime_event(message);
		}
	}
}
